...
class SysPlayerArenaFixed {
	...
	public static function  loadedTable( $fields=NULL,$condition=NULL)
	{
		...
		if (empty($condition))
		{
			$sql = "SELECT {$p} FROM `player_arena_fixed`";
		}
		else
		{			
			$sql = "SELECT {$p} FROM `player_arena_fixed` WHERE ".SQLUtil::parseCondition($condition);
		}			
	...
	public static function insertSqlHeader($fields=NULL)
	{
		...
		if(!empty($fields)){
			...
			$result[0] = "INSERT INTO `player_arena_fixed` ({$where}) VALUES ";	
			$ar = array();
			...
			$result[0]="INSERT INTO `player_arena_fixed` (`user_id`,`zone_id`,`server_id`,`rank`,`best_rank`,`reward_rank_list`,`fight_count`,`last_fight_time`,`buy_count`,`last_buy_time`,`win_count`,`lineup`,`all_gs`,`is_robot`) VALUES ";
	...
	public function  loaded( $fields=NULL,$condition=NULL)
	{
		...
		$sql = "SELECT {$p} FROM `player_arena_fixed` WHERE {$where}";
	...
	public static function sql_delete($condition=NULL)
	{
		...
		$sql = "DELETE FROM `player_arena_fixed` WHERE ".SQLUtil::parseCondition($condition);
	...
	public function  delete()
	{
		...
		$sql = "DELETE FROM `player_arena_fixed` WHERE `user_id`='{$this->user_id}'";
	...
	private function  getInSQL()
	{
		...
		$sql = "INSERT INTO `player_arena_fixed` ".$fields.$values;
	...
	private function  getUpSQL($condition)
	{
		...
		$sql = "UPDATE `player_arena_fixed` SET {$update} WHERE {$condition}";
	...
	{				
		...
		$sql = "UPDATE `player_arena_fixed` SET {$update} WHERE {$uc}";
