...
class SysPlayerVitality {
	...
	public static function  loadedTable( $fields=NULL,$condition=NULL)
	{
		...
		if (empty($condition))
		{
			$sql = "SELECT {$p} FROM `player_vitality`";
		}
		else
		{			
			$sql = "SELECT {$p} FROM `player_vitality` WHERE ".SQLUtil::parseCondition($condition);
		}			
	...
	public static function insertSqlHeader($fields=NULL)
	{
		...
		if(!empty($fields)){
			...
			$result[0] = "INSERT INTO `player_vitality` ({$where}) VALUES ";	
			$ar = array();
			...
			$result[0]="INSERT INTO `player_vitality` (`user_id`,`cur_vitality`,`last_change_time`,`today_buy`,`last_buy_time`) VALUES ";
			$result[1] = array('user_id'=>1,'cur_vitality'=>1,'last_change_time'=>1,'today_buy'=>1,'last_buy_time'=>1);
		}				
		return $result;
	}
	...
	public function  loaded( $fields=NULL,$condition=NULL)
	{
		...
		$sql = "SELECT {$p} FROM `player_vitality` WHERE {$where}";
	...
	public static function sql_delete($condition=NULL)
	{
		...
		$sql = "DELETE FROM `player_vitality` WHERE ".SQLUtil::parseCondition($condition);
	...
	public function  delete()
	{
		...
		$sql = "DELETE FROM `player_vitality` WHERE `user_id`='{$this->user_id}'";
	...
	private function  getInSQL()
	{
		...
		$sql = "INSERT INTO `player_vitality` ".$fields.$values;
	...
	private function  getUpSQL($condition)
	{
		...
		$sql = "UPDATE `player_vitality` SET {$update} WHERE {$condition}";
	...
	{				
		...
		$sql = "UPDATE `player_vitality` SET {$update} WHERE {$uc}";
