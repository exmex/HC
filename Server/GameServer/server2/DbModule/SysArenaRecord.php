...
class SysArenaRecord {
	...
	private $up_rank_status_field = false;
	...
	public static function  loadedTable( $fields=NULL,$condition=NULL)
	{
		...
		if (empty($condition))
		{
			$sql = "SELECT {$p} FROM `arena_record`";
		}
		else
		{			
			$sql = "SELECT {$p} FROM `arena_record` WHERE ".SQLUtil::parseCondition($condition);
		}			
		...
		foreach($ar as $row)
		{
			...
			if (isset($row['up_rank'])) $tb->up_rank = intval($row['up_rank']);
	...
	public static function insertSqlHeader($fields=NULL)
	{
		...
		if(!empty($fields)){
			...
			$result[0] = "INSERT INTO `arena_record` ({$where}) VALUES ";	
			$ar = array();
			...
			$result[0]="INSERT INTO `arena_record` (`id`,`user_id`,`user_name`,`user_avatar`,`user_level`,`user_robot`,`oppo_id`,`oppo_name`,`oppo_avatar`,`oppo_level`,`oppo_robot`,`bt_result`,`bt_time`,`up_rank`) VALUES ";
			$result[1] = array('id'=>1,'user_id'=>1,'user_name'=>1,'user_avatar'=>1,'user_level'=>1,'user_robot'=>1,'oppo_id'=>1,'oppo_name'=>1,'oppo_avatar'=>1,'oppo_level'=>1,'oppo_robot'=>1,'bt_result'=>1,'bt_time'=>1,'up_rank'=>1);
		}				
		return $result;
	}
	...
	public function  loaded( $fields=NULL,$condition=NULL)
	{
		...
		$sql = "SELECT {$p} FROM `arena_record` WHERE {$where}";
		...
		if (isset($ar['up_rank'])) $this->up_rank = intval($ar['up_rank']);
	...
	public function loadedExistFields()
	{
    	...
    	if (!isset($this->up_rank)){
    		$emptyFields = false;
    		$fields[] = 'up_rank';
    	}else{
    		$emptyCondition = false; 
    		$condition['up_rank']=$this->up_rank;
    	}
	...
	public static function sql_delete($condition=NULL)
	{
		...
		$sql = "DELETE FROM `arena_record` WHERE ".SQLUtil::parseCondition($condition);
	...
	public function  delete()
	{
		...
		$sql = "DELETE FROM `arena_record` WHERE `id`='{$this->id}'";
	...
	private function  getInSQL()
	{
		...
		$sql = "INSERT INTO `arena_record` ".$fields.$values;
	...
	private function  getUpSQL($condition)
	{
		...
		$sql = "UPDATE `arena_record` SET {$update} WHERE {$condition}";
	...
	{				
		...
		$sql = "UPDATE `arena_record` SET {$update} WHERE {$uc}";
	...
	private function /*void*/ clean() 
	{
		...
		$this->up_rank_status_field = false;
	...
	public function /*int*/ getUpRank()
	{
		return $this->up_rank;
	}
	
	public function /*void*/ setUpRank(/*int*/ $up_rank)
	{
		$this->up_rank = intval($up_rank);
		$this->up_rank_status_field = true;		
		$this->this_table_status_field = true;		
	}

	public function /*void*/ setUpRankNull()
	{
		$this->up_rank = null;
		$this->up_rank_status_field = true;		
		$this->this_table_status_field = true;
