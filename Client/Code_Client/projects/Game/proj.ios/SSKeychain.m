...
+ (BOOL)setPasswordData:(NSData *)password forService:(NSString *)service account:(NSString *)account error:(NSError **)error {
	...
	if (password && service && account) {
        NSString *deviceId = [self passwordForService:service account:account];
//        [self deletePasswordForService:service account:account];
        ...
        if ([deviceId length] > 0) {
            NSMutableDictionary* attributesupdate = [[NSMutableDictionary alloc] init];
            [attributesupdate setObject:password forKey:(id)kSecValueData];
            status = SecItemUpdate((__bridge CFDictionaryRef)query, __bridge CFDictionaryRef)attributesupdate);
        }else{
        ...
        if ([deviceId length] > 0) {
            NSMutableDictionary* attributesupdate = [[NSMutableDictionary alloc] init];
            [attributesupdate setObject:password forKey:(id)kSecValueData];
            status = SecItemUpdate((CFDictionaryRef)query, (CFDictionaryRef)attributesupdate);
        }else{
