package com.youai.dota.platform.ydmm;
...
public class PlatformMMLoginAndPay implements IPlatformLoginAndPay {

	
private static final String TAG = PlatformMMLoginAndPay.class.getSimpleName();
	...
	private static PlatformMMLoginAndPay sInstance = null;
	public static PlatformMMLoginAndPay getInstance() {
		if (sInstance == null) {
			sInstance = new PlatformMMLoginAndPay();
		}
		return sInstance;
	}
	
	private PlatformMMLoginAndPay() {
	...
	public void init(IGameActivity game_acitivity, GameInfo game_info) {
		...
		game_info.platform_type_str = PlatformAndGameInfo.enPlatformName_YDMM;
		game_info.platform_type = PlatformAndGameInfo.enPlatform_Ydmm;
	...
	public int callPayRecharge(PayInfo pay_info) {
		...
		YouaiPay.MMAPPID = this.game_info.pay_id_str;
		YouaiPay.MMAPPKEY = this.game_info.private_str;
		...
		YouaiCommplatform.getInstance().youaiEnterRecharge(game_ctx, payParams,new CallbackListener() {
			...
			public void onPaymentSuccess() {
				game_ctx.runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						notifyPayRechargeRequestResult(sInstance.pay_info);
						Toast.makeText(PlatformMMLoginAndPay.this.game_ctx, "提交成功", Toast.LENGTH_SHORT).show();		
					}
				});
			...
			public void onPaymentError() {
				game_ctx.runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						Toast.makeText(PlatformMMLoginAndPay.this.game_ctx, "提交失败", Toast.LENGTH_SHORT).show();		
					}
				});
			}
