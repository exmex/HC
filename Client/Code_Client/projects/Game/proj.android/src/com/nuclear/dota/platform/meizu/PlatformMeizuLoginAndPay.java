package com.nuclear.dota.platform.meizu;
...
public class PlatformMeizuLoginAndPay implements IPlatformLoginAndPay {

	
	private static final String TAG = PlatformMeizuLoginAndPay.class.getSimpleName();
	...
	private static PlatformMeizuLoginAndPay sInstance = null;
	public static PlatformMeizuLoginAndPay getInstance() {
		if (sInstance == null) {
			sInstance = new PlatformMeizuLoginAndPay();
		}
		return sInstance;
	}
	
	private PlatformMeizuLoginAndPay() {
	...
	public void init(IGameActivity game_acitivity, GameInfo game_info) {
		...
		game_info.platform_type_str = PlatformAndGameInfo.enPlatformName_Meizu;
		game_info.platform_type = PlatformAndGameInfo.enPlatform_Meizu;
		...
		YouaiCommplatform.getInstance().youaiEnterRecharge(game_ctx, payParams,new CallbackListener() {
			...
			public void onPaymentSuccess() {
				game_ctx.runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						Toast.makeText(PlatformMeizuLoginAndPay.this.game_ctx, "提交成功", Toast.LENGTH_SHORT).show();		
					}
				});
			...
			public void onPaymentError() {
				game_ctx.runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						Toast.makeText(PlatformMeizuLoginAndPay.this.game_ctx, "提交失败", Toast.LENGTH_SHORT).show();		
					}
				});
			}
