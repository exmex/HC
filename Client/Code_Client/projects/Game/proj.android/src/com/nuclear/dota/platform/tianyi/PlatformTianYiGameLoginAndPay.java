package com.nuclear.dota.platform.tianyi;
...
public class PlatformTianYiGameLoginAndPay implements IPlatformLoginAndPay {
	private static final String TAG = PlatformTianYiGameLoginAndPay.class.getSimpleName();
	...
	private static PlatformTianYiGameLoginAndPay sInstance = null;
	public static PlatformTianYiGameLoginAndPay getInstance() {
		if (sInstance == null) {
			sInstance = new PlatformTianYiGameLoginAndPay();
		}
		return sInstance;
	}
	
	private PlatformTianYiGameLoginAndPay() {
	...
	public void init(IGameActivity game_acitivity, GameInfo game_info) {
		...
		game_info.platform_type_str = PlatformAndGameInfo.enPlatformName_TianYi;
		game_info.platform_type = PlatformAndGameInfo.enPlatform_TianYi;
		...
		YouaiCommplatform.getInstance().youaiEnterRecharge(game_ctx, payParams,new CallbackListener() {
			...
			public void onPaymentSuccess() {
				game_ctx.runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						notifyPayRechargeRequestResult(PlatformTianYiGameLoginAndPay.sInstance.pay_info);
						Toast.makeText(PlatformTianYiGameLoginAndPay.this.game_ctx, "提交成功", Toast.LENGTH_SHORT).show();		
					}
				});
			...
			public void onPaymentError() {
				game_ctx.runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						Toast.makeText(PlatformTianYiGameLoginAndPay.this.game_ctx, "提交失败", Toast.LENGTH_SHORT).show();		
					}
				});
			}
