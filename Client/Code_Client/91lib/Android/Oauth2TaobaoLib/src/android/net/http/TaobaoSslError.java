...
public class TaobaoSslError {
  ...
  public static final int SSL_NOTYETVALID = 0;
     ...
     */
    public TaobaoSslError(int error, SslCertificate certificate) {
        addError(error);
     ...
     * @param certificate The associated SSL certificate
     */
    public TaobaoSslError(int error, X509Certificate certificate) {
        addError(error);
    ...
    public boolean addError(int error) {
        boolean rval = (0 <= error && error < TaobaoSslError.SSL_MAX_ERROR);
        if (rval) {
            mErrors |= (0x1 << error);
        }
    ...
    public int getPrimaryError() {
        if (mErrors != 0) {
            ...
            for (int error = TaobaoSslError.SSL_MAX_ERROR - 1; error >= 0; --error) {
