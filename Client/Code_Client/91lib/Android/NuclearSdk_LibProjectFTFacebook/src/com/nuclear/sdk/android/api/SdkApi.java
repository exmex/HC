...
import com.nuclear.sdk.net.HttpManager;
...
public class SdkApi {
	public static String TAG = "nuclearApi";
		...
		new Thread(){
			public void run() {
				 ...
				 String tempStr = null;
					...
					String md5sign = MD5.sign(putmessage, "-128315");
			        try {
			            url = new URL(SdkConfig.precreate_url);
			        	...
			        	listener.onError(new SdkException(e));
					} catch (IOException e) {
		...
		new Thread(){
			public void run() {
				 String tempStr = null;
			        ...
			        HttpURLConnection url_con = null;
			        try {
			            url = new URL(SdkConfig.modify_url);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            ...
			            listener.onComplete(backjson);
			            	...
			            	listener.onError(new SdkException(code));
			            }
			        } catch (MalformedURLException e) {
			        	listener.onError(new SdkException(e));
					} catch (IOException e) {
	...
	public static void findPass(final String param,final String checksum,final RequestListener listener){
		...
		new Thread(){
			public void run() {
				 String tempStr = null;
			        ...
			        HttpURLConnection url_con = null;
			        try {
			            url = new URL(SdkConfig.FindPass);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
	...
	public static void getUserList(final String param,final RequestListener listener){
			        ...
			        try {
			    		...
			    		checksum = MD5.sign(encript, SdkConfig.md5key);
			            url = new URL(SdkConfig.GetnuclearList);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
	...
	public static void CreateOrLoginTry(final String param,final RequestListener listener){
        ...
        try {
    		...
    		encript = RSAUtil.encryptByPubKey(param, RSAUtil.pub_key_hand);
    		checksum = MD5.sign(encript, SdkConfig.md5key);
            url = new URL(SdkConfig.createOrLoginTry);
            url_con = (HttpURLConnection) url.openConnection();
            url_con.setRequestMethod("PUT");
            url_con.addRequestProperty("Game-Checksum", checksum);
            url_con.setDoOutput(true);
	...
	public static void getnuclearNames(final String param,final RequestListener listener){
		new Thread(){
			public void run() {
		 String tempStr = null;
	        URL url;
	        ...
	        try {
	        	final String checksum;
	    		checksum = MD5.sign(param, SdkConfig.md5key);
	            url = new URL(SdkConfig.GetnuclearList);
	            url_con = (HttpURLConnection) url.openConnection();
	            url_con.setRequestMethod("PUT");
	            url_con.addRequestProperty("Game-Checksum", checksum);
			...
			public void run() {
				 String tempStr = null;
			        ...
			        try {
			    		...
			    		encript = RSAUtil.encryptByPubKey(param, RSAUtil.pub_key_hand);
			    		checksum = MD5.sign(encript, SdkConfig.md5key);
			            url = new URL(SdkConfig.pushforclient);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
			...
			public void run() {
				 String tempStr = null;
			        ...
			        try {
			    		...
			    		encript = RSAUtil.encryptByPubKey(param, RSAUtil.pub_key_hand);
			    		checksum = MD5.sign(encript, SdkConfig.md5key);
			            url = new URL(SdkConfig.login_url);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
			...
			public void run() {
				 String tempStr = null;
			        ...
			        try {
			    		...
			    		encript = RSAUtil.encryptByPubKey(param, RSAUtil.pub_key_hand);
			    		checksum = MD5.sign(encript, SdkConfig.md5key);
			            url = new URL(SdkConfig.TryBinding);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
			...
			public void run() {
				 String tempStr = null;
			        ...
			        try {
			    		...
			    		checksum = MD5.sign(encript, SdkConfig.md5key);
			            url = new URL(SdkConfig.Thirdlogin_url);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
		...
		new Thread(){
			public void run() {
				 String tempStr = null;
			        ...
			        HttpURLConnection url_con = null;
			        try {
			            url = new URL(SdkConfig.createUser_url);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", Checksum);
			            url_con.setDoOutput(true);
			            ...
			            listener.onComplete(tempStr);
			            	...
			            	listener.onError(new SdkException(statuscode));
			            }
			        } catch (MalformedURLException e) {
			        	listener.onError(new SdkException(e));
					} catch (IOException e) {
	...
	public static void Init(final Context pContext,final String appid,final String appKey,final String channelName,final RequestListener listener){
		...
		new Thread() {
			public void run() {
				...
				String md5sign = MD5.sign(putmessage, "-128315");
		        try {
		            url = new URL(SdkConfig.init_url);
		            url_con = (HttpURLConnection) url.openConnection();
		            url_con.setRequestMethod("PUT");
		            url_con.addRequestProperty("Game-Checksum", md5sign);
...
public static void requestTest(){
	...
	new Thread() {
		public void run() {
			...
			HttpPut post = new HttpPut(SdkConfig.precreate_url);
			try {
