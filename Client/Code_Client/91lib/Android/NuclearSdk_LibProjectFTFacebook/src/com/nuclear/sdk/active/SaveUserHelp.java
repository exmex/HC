...
import com.nuclear.sdk.android.api.DatabaseHelper;
 ...
 *   String UPDATA_DATA = "UPDATE nuclear SET text='通过SQL语句来修改数据'  WHERE id=1"; 
 *                  千万 不能可以写成小写
 */
...
//---------------以下两个成员变量是针对在SD卡中存储数据库文件使用
	private File pathDb =new File( Environment.getExternalStorageDirectory()
			+ File.separator + "nuclear_league" + File.separator);
	private File fileDb = new File( Environment.getExternalStorageDirectory()
			+ File.separator + "nuclear_league" + File.separator+DatabaseHelper.DATABASE_NAME);
	...
	public void onCreate() {
		...
		//try{
			my_SqlDb = SQLiteDatabase.openOrCreateDatabase(fileDb, null);	
			 String tableExist =
					 "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='"+DatabaseHelper.TABLE_NAME+"'";
	...
	//添加数据
	public void doDbInsert(SdkUser pUser) {
		if (!fileDb.exists()) {
			return ;
		}
		String password = "";
		...
		try { 
			...
			//备注2----如果你使用的是将数据库的文件创建在SD卡中，那么创建数据库mysql如下操作：
							// ---------------------- SQL语句插入--------------
			String INSERT_DATA =
			"INSERT INTO "+DatabaseHelper.TABLE_NAME+"(username,password,email,userType,uidStr,isLocalLogout,isFirstThird,lastLoginTime) values ('"+pUser.getUsername()
			+"','"+password
			+"','"+pUser.getEmail()
			+"','"+pUser.getUserType()
			+"','"+pUser.getUidStr()
	...
	public ArrayList<SdkUser> doDbSelectAll(){
		ArrayList<SdkUser> users = new ArrayList<SdkUser>();
		if (!fileDb.exists()) {
			return users;
		}
		...
		try{
			my_SqlDb = SQLiteDatabase.openOrCreateDatabase(fileDb, null); 
			Cursor cur = my_SqlDb.rawQuery("SELECT * FROM "+DatabaseHelper.TABLE_NAME+" ORDER BY lastLoginTime DESC", null);
			if (cur != null) {
				while (cur.moveToNext()) {//直到返回false说明表中到了数据末尾
					SdkUser _user = new SdkUser();
	...
	public void doDbDelete(SdkUser pUser){
		if (!fileDb.exists()) {
			return ;
		}
		try { 
		 my_SqlDb = SQLiteDatabase.openOrCreateDatabase(fileDb, null); 
				// ---------------------- 读写句柄来删除
				my_SqlDb.delete(DatabaseHelper.TABLE_NAME, DatabaseHelper.uidStr + "= '"+pUser.getUidStr()+"'", null);
				// 第一个参数 需要操作的表名
				// 第二个参数为 id+操作的下标 如果这里我们传入null，表示全部删除
				// 第三个参数默认传null即可
				// ----------------------- SQL语句来删除
				// String DELETE_DATA = "DELETE FROM nuclear WHERE id=1";
	...
	public void doDbUpdate(SdkUser pUser){
		if (!fileDb.exists()) {
			return ;
		}
		String password = "";
		...
		try { 
			my_SqlDb = SQLiteDatabase.openOrCreateDatabase(fileDb, null); 
				// ------------------------SQL语句来修改 -------------
				 String UPDATA_DATA =
				 "UPDATE "+DatabaseHelper.TABLE_NAME+" SET lastLoginTime= datetime('now') ,userType='"+pUser.getUserType()+"', password='"+password
	...
	public void doDbUpdateOrInsert(SdkUser pUser){
		if (!fileDb.exists()) {
			return ;
		}
		String password = "";
		...
		try { 
			my_SqlDb = SQLiteDatabase.openOrCreateDatabase(fileDb, null); 
				// ------------------------SQL语句来修改 -------------
			Cursor cur = my_SqlDb.rawQuery("SELECT * FROM "+DatabaseHelper.TABLE_NAME+" WHERE uidStr='"+pUser.getUidStr()+"'", null);
			if (cur != null&cur.moveToNext()) {
				String UPDATA_DATA ;
				if(password.equals("")){

					UPDATA_DATA  =
							 "UPDATE "+DatabaseHelper.TABLE_NAME+" SET lastLoginTime= datetime('now') ,userType='"+pUser.getUserType()+"', username='"+pUser.getUsername()+"', email='"+pUser.getEmail()+"', isLocalLogout='"+pUser.getIsLocalLogout()
	...
	public void doDbDelTable(){
		...
		try { 
		my_SqlDb = SQLiteDatabase.openOrCreateDatabase(fileDb, null); 
				my_SqlDb.execSQL("DROP TABLE nuclear");
		} catch (Exception e) {
			Log.e("SaveTestHelp", e.toString());
		} finally {// 如果try中异常，也要对数据库进行关闭
			my_SqlDb.close();
