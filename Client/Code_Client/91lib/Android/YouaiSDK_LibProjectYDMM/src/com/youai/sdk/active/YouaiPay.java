...
public class YouaiPay  extends Activity{
	...
	private ImageView YdmmPay;
	private AliPayClick apayClick;
	private TencentClick tpayClick;
	...
	public static final int PayErrorNet = 2001;	public static final int PayErrorRequest = 2002;
	...
	private boolean ydmmInitOk = false;
	...
	// 计费点信息
	private static final String MMLEASE_PAYCODE[] = {"30000394605301","30000394605303","30000394605304","30000394605302"}; 
	...
	private  boolean lowThirty = false;//充值是否低于30元
	...
	protected void onCreate(Bundle savedInstanceState) {
		 ...
		 if(mPayinfo.getMoney()<=30){//移动MM支付不支持30元以上
			 lowThirty = true;
			 if(mPayinfo.getMoney()==30){
				 atPayCode = 3;
			 }else if(mPayinfo.getMoney()==20){
				 atPayCode = 2;
			 }else if(mPayinfo.getMoney()==10){
				 atPayCode = 1;
			 }else {
				 atPayCode = 0;
			 }
		 ...
		 if(!lowThirty){
			 ...
			 findViewById(R.id.u2_charge_rl_ydmm).setVisibility(View.GONE);
		 }else{
			 ...
			 if(null!=operator){
				 ...
				 if(operator.startsWith("46000") || operator.startsWith("46002")|| operator.startsWith("46007")){  
					 //中国移动  
				 lowThirty = true;
				 YdmmPay = (ImageView) findViewById(R.id.u2_charge_ydmm);
				 }else{
					 lowThirty = false;
					 findViewById(R.id.u2_charge_rl_ydmm).setVisibility(View.GONE);
				 }
		 ...
		 if(lowThirty == false)//移动MM支付不支持30元以上的
			 return;
		 
		 mProgressDialog = new ProgressDialog(YouaiPay.this);
		 mProgressDialog.setIndeterminate(true);
		 mProgressDialog.setMessage("请稍候...");
		
		 
		 IAPHandler iapHandler = new IAPHandler(YouaiPay.this);
			/**
			 * IAP组件初始化.包括下面3步。
			 */
			/**
			 ...
			 */
			mListener = new IAPListener(YouaiPay.this, iapHandler);
			/**
			 * step2.获取Purchase实例。
			 */
			 ...
			 */
			try {
				purchase.init(YouaiPay.this, mListener);
				ydmmInitOk = true;
			} catch (Exception e) {
				e.printStackTrace();
				ydmmInitOk = false;
		...
		showProgressDialog();
		YdmmPay.setOnClickListener(new YdmmPayClick());
	...
	class TencentClick implements OnClickListener{
	...
	class YdmmPayClick implements OnClickListener{

		@Override
		public void onClick(View arg0) {
			try {
				purchase.order(YouaiPay.this,  MMLEASE_PAYCODE[atPayCode], 1, mPayinfo.getExtraInfo() ,mListener);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
