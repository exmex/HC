...
public class YouaiApi {
		...
		new Thread(){
			public void run() {
				 ...
				 String tempStr = null;
			        ...
			        try {
			            url = new URL(YouaiSdkConfig.precreate_url);
		...
		new Thread(){
			public void run() {
				 String tempStr = null;
			        ...
			        HttpURLConnection url_con = null;
			        try {
			            url = new URL(YouaiSdkConfig.modify_url);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
		...
		new Thread(){
			public void run() {
				 String tempStr = null;
			        ...
			        HttpURLConnection url_con = null;
			        try {
			            url = new URL(YouaiSdkConfig.FindPass);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
	...
	public static void getUserList(final String param,final RequestListener listener){
			        ...
			        try {
			    		...
			    		checksum = MD5.sign(encript, YouaiSdkConfig.md5key);
			            url = new URL(YouaiSdkConfig.GetYouaiList);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
	...
	public static void CreateOrLoginTry(final String param,final RequestListener listener){
        ...
        try {
    		...
    		encript = RSAUtil.encryptByPubKey(param, RSAUtil.pub_key_hand);
    		checksum = MD5.sign(encript, YouaiSdkConfig.md5key);
            url = new URL(YouaiSdkConfig.createOrLoginTry);
            url_con = (HttpURLConnection) url.openConnection();
            url_con.setRequestMethod("PUT");
            url_con.addRequestProperty("Game-Checksum", checksum);
            url_con.setDoOutput(true);
	...
	public static void getYouaiNames(final String param,final RequestListener listener){
		new Thread(){
		 ...
		 String tempStr = null;
	        ...
	        try {
	        	final String checksum;
	    		checksum = MD5.sign(param, YouaiSdkConfig.md5key);
	            url = new URL(YouaiSdkConfig.GetYouaiList);
	            url_con = (HttpURLConnection) url.openConnection();
	            url_con.setRequestMethod("PUT");
	            url_con.addRequestProperty("Game-Checksum", checksum);
			...
			public void run() {
				 String tempStr = null;
			        ...
			        try {
			    		...
			    		encript = RSAUtil.encryptByPubKey(param, RSAUtil.pub_key_hand);
			    		checksum = MD5.sign(encript, YouaiSdkConfig.md5key);
			            url = new URL(YouaiSdkConfig.pushforclient);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
			...
			public void run() {
				 String tempStr = null;
			        ...
			        try {
			    		...
			    		encript = RSAUtil.encryptByPubKey(param, RSAUtil.pub_key_hand);
			    		checksum = MD5.sign(encript, YouaiSdkConfig.md5key);
			            url = new URL(YouaiSdkConfig.login_url);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
			...
			public void run() {
				 String tempStr = null;
			        ...
			        try {
			    		...
			    		encript = RSAUtil.encryptByPubKey(param, RSAUtil.pub_key_hand);
			    		checksum = MD5.sign(encript, YouaiSdkConfig.md5key);
			            url = new URL(YouaiSdkConfig.TryBinding);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
		...
		public void run() {
			 String tempStr = null;
		        ...
		        try {
		    		...
		    		encript = RSAUtil.encryptByPubKey(param, RSAUtil.pub_key_hand);
		    		checksum = MD5.sign(encript, YouaiSdkConfig.md5key);
		            url = new URL(YouaiSdkConfig.TryToOk);
		            url_con = (HttpURLConnection) url.openConnection();
		            url_con.setRequestMethod("PUT");
		            url_con.addRequestProperty("Game-Checksum", checksum);
		            url_con.setDoOutput(true);
			...
			public void run() {
				 String tempStr = null;
			        ...
			        try {
			    		...
			    		checksum = MD5.sign(encript, YouaiSdkConfig.md5key);
			            url = new URL(YouaiSdkConfig.Thirdlogin_url);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", checksum);
			            url_con.setDoOutput(true);
		...
		new Thread(){
			public void run() {
				 String tempStr = null;
			        ...
			        HttpURLConnection url_con = null;
			        try {
			            url = new URL(YouaiSdkConfig.createUser_url);
			            url_con = (HttpURLConnection) url.openConnection();
			            url_con.setRequestMethod("PUT");
			            url_con.addRequestProperty("Game-Checksum", Checksum);
			            url_con.setDoOutput(true);
	...
	public static void Init(final Context pContext,final String appid,final String appKey,final String channelName,final RequestListener listener){
		...
		new Thread() {
			public void run() {
				...
				String md5sign = MD5.sign(putmessage, "-com4loves");
		        try {
		            url = new URL(YouaiSdkConfig.init_url);
		            url_con = (HttpURLConnection) url.openConnection();
		            url_con.setRequestMethod("PUT");
		            url_con.addRequestProperty("Game-Checksum", md5sign);
...
public static void requestTest(){
	...
	new Thread() {
		public void run() {
			...
			HttpPut post = new HttpPut(YouaiSdkConfig.precreate_url);
			try {
