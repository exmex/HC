...
public class YouaiCommplatform {
	...
	public void Init(final Activity activity, YouaiAppInfo youaiAppInfo,
		...
		if ((youaiAppInfo == null) || (youaiAppInfo.appId<0)|| (youaiAppInfo.appSecret == null)
			...
			initOk = false;
			mOnInitCallbackListener.onFailed(YouaiErrorCode.YOUAI_INIT_ERROR, activity.getString(R.string.initfailinner));
			return;
		}
		YouaiControlCenter.mCallBack = new CallbackListener(){
		...
		AsyncUserRunner.requestInit(activity,String.valueOf(mAppInfo.getAppId()),mAppInfo.getAppKey(),channelName, new RequestListener() {
			...
			public void onIOException(IOException e) {
				...
				if(initTime<4){
					if(initTime==2){
						YouaiSdkConfig.urlroot = YouaiSdkConfig.urlroot_back;
					}
				...
				activity.runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						Toast.makeText(activity, activity.getString(R.string.initfailtryagain), Toast.LENGTH_SHORT).show();
			...
			public void onError(YouaiException e) {
				...
				activity.runOnUiThread(new Runnable() {
					...
					public void run() {
						Toast.makeText(activity, activity.getString(R.string.initfailtryagain), Toast.LENGTH_SHORT).show();
						if(initTime<4)return;
						mOnInitCallbackListener.onFailed(YouaiErrorCode.YOUAI_INIT_SUCCESS, activity.getString(R.string.initfailinner));
					}
				});
			}
			...
			public void onComplete(String response) {
				...
				try {
					...
					if(code.equals("200")){
						YouaiSdkConfig.timestamp = backjson.getLong("timestamp");
						 YouaiSdkConfig.UrlFeedBack += "?gameId="+mAppInfo.getAppKey() + "&platformId="+YouaiSdkConfig.PlatformStr;
						initOk = true;
						activity.runOnUiThread(new Runnable() {
							@Override
							public void run() {
	...
	protected void LoginTry(){
		...
		if(null!=OnSuccesBackJsonStr){
			...
			mUserCallback.onLoginError(new YouaiError(mAppInfo.getCtx().getString(R.string.netwarning)));
			isLogined = false;
		}
	...
	private void popuAlert(){
		 new AlertDialog.Builder(mAppInfo.getCtx())
			...
			.setPositiveButton(R.string.ensure, new Dialog.OnClickListener() {
				...
				public void onClick(DialogInterface arg0, int arg1) {
					...
					YouaiLogin.autoLogin = false;
					YouaiCommplatform.this.isLogined = false;
					Toast.makeText(mAppInfo.getCtx(), R.string.logoutsuccess, Toast.LENGTH_SHORT).show();
...
//					pContext.startActivity(intent);
					Intent intent = new Intent(mAppInfo.getCtx(), YouaiControlCenter.class);
					intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		...
		String userType = "";
		 if (null != OnSuccessLoginUser) {
				...
				if (userType.equals("2")) {
					...
					YouaiLogin.showRegister = true;
					intent.putExtra("KEY_POSITION", 1);
					YouaiControlCenter.mCallBack = YouaiLogin.listener;
					YouaiControlCenter.setFromContext(mAppInfo.getCtx());
					mAppInfo.getCtx().startActivity(intent);
				}
		}
	}
	...
	public void facebookLogin(String uid,String name,String token,Activity pAcitivity) throws JSONException{
		Token tokenfb = new Token();
		tokenfb.setUid(uid);
		tokenfb.setAccessToken(token);
		tokenfb.setUsername(name);
		Log.e("fbUid", uid);  
		JSONObject message = new JSONObject();
		JSONObject data = new JSONObject(); 
		data.put("thirdId", tokenfb.getUid()); 
		data.put("nickName", tokenfb.getUsername()); 
		...
		YouaiApi.thirdLogin(message, new RequestListener() {
			...
			public void onIOException(IOException e) {
				...
				pActivity.runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						Toast.makeText(pActivity, R.string.loginfail, Toast.LENGTH_SHORT).show();		
					}
				});
			...
			public void onComplete(String response) {
				...
				try {
					jsonback  = new JSONObject(response);
					error = jsonback.getString("error");
					if(error.equals(YouaiSdkConfig.statuCode)){
					...
					backjsontosdk = jsonback.toString();
					...
					pActivity.runOnUiThread(new Runnable() {
						@Override
						public void run() {
						LayoutInflater inflater = LayoutInflater.from(pActivity);
			            View view = inflater.inflate(R.layout.youai_toast, (ViewGroup)pActivity.findViewById(R.id.toast_layout_root));
			            TextView textView = (TextView) view.findViewById(R.id.welcome);
			            textView.setText(YouaiCommplatform.getInstance().getLoginNickName()+","+pActivity.getString(R.string.welcome)+" ！");
			            ...
			            YouaiLogin.listener.onLoginSuccess(backjsontosdk);
						}
						});
					}else{
						...
						pActivity.runOnUiThread(new Runnable() {
							@Override
							public void run() {
								YouaiLogin.listener.onLoginError(new YouaiError(errorMessage));
								Toast.makeText(pActivity, pActivity.getString(R.string.loginfail)+":"+errorMessage, Toast.LENGTH_SHORT).show();
							}
						});
	...
	protected void notifyBind(boolean pResult) {
		...
		if (pResult) {
			...
			new AlertDialog.Builder(mAppInfo.getCtx()).setTitle(R.string.warning)
					.setMessage(R.string.bindsuccess).setPositiveButton(R.string.ensure, null).show();
		}
	}
