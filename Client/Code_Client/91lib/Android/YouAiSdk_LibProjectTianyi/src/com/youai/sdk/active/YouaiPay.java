...
public class YouaiPay  extends Activity{
	...
	private boolean TyInitOk = false;
	 
	private ProgressDialog mProgressDialog;
	// 计费信息
	public static  String TYAPPID = "";
	public static  String TYAPPKEY = "";
	
	
	// 计费点信息
	private static final String TY_PAYCODE[] = {"E59A4C411A9797C0E040640A041E0698","30000394605303","30000394605304","30000394605302"}; 
	...
	protected void onCreate(Bundle savedInstanceState) {
		 ...
		 TianyiPaytv = (TextView) findViewById(R.id.u2_charge_tianyi_tv);
		 TianyiPay = (ImageView) findViewById(R.id.u2_charge_tianyi);
		 
		 if(mPayinfo.getMoney()<=20){//天翼支付不支持20元以上
			 lowTwenty = true;
			 if(mPayinfo.getMoney()==20){
				 atPayCode = 2;
		 ...
		 if(!lowTwenty){
			 ...
			 findViewById(R.id.u2_charge_tianyi_tv).setVisibility(View.GONE);
		 }else{
			 ...
			 if(null!=operator){
				 ...
				 if(operator.startsWith("46003")){  
					 //中国电信 
				 lowTwenty = true;
				 findViewById(R.id. tianyi_rl).setVisibility(View.VISIBLE);
				 }else{
					 lowTwenty = false;
					 findViewById(R.id. tianyi_rl).setVisibility(View.GONE);
				 }
			 }else{
				 lowTwenty = false;
				 findViewById(R.id. tianyi_rl).setVisibility(View.GONE);
		 ...
		 if(lowTwenty == false)//天翼空间支付不支持30元以上的
			 return;
		 TianyiPay.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				 Intent intent = new Intent();
	                intent.setClass(YouaiPay.this,  CTEStoreSDKActivity.class);

	                Bundle bundle = new Bundle();
	                //示例参数值
	                //YouaiPay.this.code = new String("E59A4C411A9797C0E040640A041E0698");
	                bundle.putString(ApiParameter.APPCHARGEID, TY_PAYCODE[atPayCode]);
	                bundle.putString(ApiParameter.CHANNELID, "2234");
	                bundle.putBoolean(ApiParameter.SCREENHORIZONTAL, false);//进入竖屏XML"
	                
	                bundle.putString(ApiParameter.CHARGENAME, "棒棒糖");
	                bundle.putInt(ApiParameter.PRICETYPE, 0);
	                bundle.putString(ApiParameter.PRICE, "0.01");
	 ...
	 @Override
	    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
	        ...
	        if (ApiParameter.CTESTORE_SENDSUCCESS == payResultCode
	            ...
	            Log.v("pay-success",bdl.getString(ApiParameter.REQUESTID));
	        }
	        else
	        {
