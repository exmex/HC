...
public class UserDialog extends Dialog {
	...
	static ArrayList<YouaiUser> youaiUsers = new ArrayList<YouaiUser>();
	...
	public UserDialog(Context context,AutoCompleteTextView pUsernameEd,EditText pPasswordEd) {
		...
		passwordEd = pPasswordEd;
		    ...
		    int count = youaiUsers.size();
			if(count==0){
				Toast.makeText(getContext(), "您还没有登录过帐号", Toast.LENGTH_SHORT).show();
			}
	 ...
	 class UserAdapter extends BaseAdapter{
		...
		public View getView( int position, View convertView, ViewGroup parent) {
			...
			holder.usernameTv.setOnClickListener(new View.OnClickListener() {
				...
				@Override
				public void onClick(View v) {
						usernameEd.setText(youaiUsers.get(_position).getUsername());
					try {
						passwordEd.setText(youaiUsers.get(_position).getPassword());
					} catch (Exception e) {
						e.printStackTrace();
					}
			...
			final View.OnClickListener  clickListenner = new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					savehelp.doDbDelete(youaiUsers.get(_position));
					youaiUsers.remove(_position);
			...
			holder.usernameTv.setText(youaiUsers.get(position).getUsername());
			return convertView;
