...
local function set_breakpoint(file, line)
  print("set breakpoint ".. file ..":"..line);
...
local function debug_hook(event, line)
 ...
 -- LegendLog("debug_hook call event: " .. tostring(event) .. " line: " .. tostring(line))
  ...
  elseif event == "line" then
    ...
    if (lastsource ~= caller.source) then
      ...
      local prefix= "Resources/data/";
      local prefix_pos = file:find(prefix)
      
      if prefix_pos then 
        file = string.sub(file,prefix_pos+string.len(prefix),-1)
      end 
      
      file = string.gsub(file,prefix,"");
      
     -- print("debug_hook call event: " .. tostring(event) .. " line: " .. tostring(line) .. " file: " .. file);
...
local function start(controller_host, controller_port)
  ...
  --LegendLog("controller_host "..controller_host.. " controller_port " .. controller_port .. " socket connect:".. tostring(socket.connect));
  local err
