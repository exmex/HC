...
unsigned int SimpleAudioEngineOpenSL::playEffect(const char* pszFilePath, bool bLoop)
{
	...
	do 
	{
		std::string mapPath = cocos2d::CCFileUtils::sharedFileUtils()->fullPathForAudioFile(pszFilePath);

		soundID = s_pOpenSL->preloadEffect(mapPath.c_str());
		if (soundID != FILE_NOT_FOUND)
		{
			if (s_pOpenSL->getEffectState(soundID) == PLAYSTATE_PLAYING)
			{
				// recreate an effect player
				s_pOpenSL->recreatePlayer(mapPath.c_str());
				break;
			}
...
{
	std::string mapPath = cocos2d::CCFileUtils::sharedFileUtils()->fullPathForAudioFile(pszFilePath);
	s_pOpenSL->preloadEffect(mapPath.c_str());
}

void SimpleAudioEngineOpenSL::unloadEffect(const char* pszFilePath)
{
	std::string mapPath = cocos2d::CCFileUtils::sharedFileUtils()->fullPathForAudioFile(pszFilePath);
	s_pOpenSL->unloadEffect(mapPath.c_str());
}
