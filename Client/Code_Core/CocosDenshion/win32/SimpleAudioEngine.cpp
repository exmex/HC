...
void SimpleAudioEngine::playBackgroundMusic(const char* pszFilePath, bool bLoop)
{
	...
	CCLOG(msg.c_str());
	CCFileUtils* fileUrl = CCFileUtils::sharedFileUtils();
	if (fileUrl)
	{
		std::string pathStr = fileUrl->fullPathForFilename(pszFilePath, EncFileNameFlag, EncFilePath, ShowSuffixFlag);
		if (!pathStr.compare(pszFilePath))
		{
			CCLOG("Get data from file( %s ) failed!",pszFilePath);
...
static std::string _FullPath(const char * szPath)
{
	return CCFileUtils::sharedFileUtils()->fullPathForAudioFile(szPath);
}
