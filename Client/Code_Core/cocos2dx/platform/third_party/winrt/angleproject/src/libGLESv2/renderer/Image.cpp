...
//yinlong added begin. be attention ,the Order of color in one byte is opposite  .2014/5/9
void Image::loadRGBA4444DataToBGRA4444(GLsizei width, GLsizei height,
	int inputPitch, const void *input, size_t outputPitch, void *output)
{
	const unsigned short *source = NULL;
	unsigned char *dest = NULL;
 	...
 	for (int y = 0; y < height; y++)
 	{
		...
		for (int x = 0; x < width; x++)
		{
			...
			dest[2 * x + 0] = ((rgba & 0x0F00) >> 4) | ((rgba & 0x00F0) >> 4);
			dest[2 * x + 1] = ((rgba & 0x000F) << 4) | ((rgba & 0xF000) >> 12);
		}
 	}
}
//yinlong added end.
