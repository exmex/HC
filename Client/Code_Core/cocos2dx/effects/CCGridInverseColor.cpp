...
#include "shaders/shaderhelper/CCInverseColorSH.h"

NS_CC_BEGIN

// implementation of CCGrid3D

 CCGridInverseColor*  CCGridInverseColor::create(const CCSize& gridSize, CCTexture2D *pTexture, bool bFlipped)
{
	 CCGridInverseColor *pRet = new  CCGridInverseColor();
 ...
 CCGridInverseColor*  CCGridInverseColor::create(const CCSize& gridSize)
{
	 CCGridInverseColor *pRet = new  CCGridInverseColor();
 ...
 CCGridInverseColor:: CCGridInverseColor()
    : m_pTexCoordinates(NULL)
    , m_pVertices(NULL)
    , m_pOriginalVertices(NULL)
    , m_pIndices(NULL)
{
	m_Mat =new CCInverseColorSH(CCShaderCache::sharedShaderCache()->programForKey(kCCShader_Inverse_Color));
	m_fActiveTime = 0.0;
}

 CCGridInverseColor::~ CCGridInverseColor(void)
{
	if (m_Mat)
	{
		delete m_Mat;
...
CCInverseColorSH*  CCGridInverseColor::GetMat()
{
	return m_Mat;
}

void CCGridInverseColor::setActiveForTime(float time){
	if (time>0.0)
	{
		m_fActiveTime = time;
		setActive(true);
...
void CCGridInverseColor::update(float dt){
	if (m_fActiveTime>0.0)
	{
		m_fActiveTime -=dt;
		if (m_fActiveTime<=0.0)
		{
...
void  CCGridInverseColor::blit(void)
{
	...
	m_pShaderProgram = CCShaderCache::sharedShaderCache()->programForKey(kCCShader_Inverse_Color);
	m_Mat->setInverseTexture(m_pTexture);
	m_Mat->begin();
	//
	// Attributes
	//
...
void  CCGridInverseColor::calculateVertexPoints(void)
{
...
ccVertex3F  CCGridInverseColor::vertex(const CCPoint& pos)
{
...
ccVertex3F  CCGridInverseColor::originalVertex(const CCPoint& pos)
{
    CCAssert( pos.x == (unsigned int)pos.x && pos.y == (unsigned int) pos.y , "Numbers must be integers");
...
void  CCGridInverseColor::setVertex(const CCPoint& pos, const ccVertex3F& vertex)
{
    CCAssert( pos.x == (unsigned int)pos.x && pos.y == (unsigned int) pos.y , "Numbers must be integers");
    int index = (pos.x * (m_sGridSize.height + 1) + pos.y) * 3;
    float *vertArray = (float*)m_pVertices;
...
void  CCGridInverseColor::reuse(void)
{
    if (m_nReuseGrid > 0)
    {
        memcpy(m_pOriginalVertices, m_pVertices, (m_sGridSize.width+1) * (m_sGridSize.height+1) * sizeof(ccVertex3F));
