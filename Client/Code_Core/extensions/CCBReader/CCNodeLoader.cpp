...
float * CCNodeLoader::parsePropTypeScaleLock(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader, const char *pPropertyName) {
	...
	do 
	{
		...
		if ( pSprite )
		{
			CCLabelTTF* ttf = dynamic_cast<CCLabelTTF*>(pNode);
			CC_BREAK_IF(ttf);
		}
		else
		{
			CCMenuItemImage* itemImage = dynamic_cast<CCMenuItemImage*>(pNode);
			CC_BREAK_IF(!itemImage);
		}
...
CCNode * CCNodeLoader::parsePropTypeCCBFile(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader) {
    std::string ccbFileName = pCCBReader->getCCBFilePath() + pCCBReader->readCachedString();

    /* Change path extension to .ccbi. */
    std::string ccbFileWithoutPathExtension = CCBReader::deletePathExtension(ccbFileName.c_str());
    ccbFileName = ccbFileWithoutPathExtension + ".ccbi";
    
	//不进行多目录查找，直接到对应目录找相应文件 by liulongfei
	//std::string filePath = "ccbi/" + ccbFileName;
    // Load sub file
    std::string path = LegendFindFileCpp(ccbFileName.c_str());
...
CCNode * CCNodeLoader::parsePropTypeCCBFileNew(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader) {
	...
	// if it has the ccbfile parent, may has bug, comment by zhenhui 2014/7/9
