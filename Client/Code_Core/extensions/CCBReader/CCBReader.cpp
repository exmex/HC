...
, mCCBName("")
{
    ...
    this->mCCBFilePath = pCCBReader->getCCBFilePath();
...
void CCBReader::setCCBFilePath(const char* pCCBFilePath)
{
    CCAssert(pCCBFilePath != NULL, "");
    mCCBFilePath = pCCBFilePath;
}

const std::string& CCBReader::getCCBFilePath() const
{
    return mCCBFilePath;
}
...
CCNode* CCBReader::readNodeGraphFromFile(const char *pCCBFileName, CCObject *pOwner, const CCSize &parentSize)
{
	...
	std::string strPath = CCFileUtils::sharedFileUtils()->fullPathForFilename(strCCBFileName.c_str(), EncFileNameFlag, EncFilePath, ShowSuffixFlag);
    unsigned long size = 0;

	static CCDictionary CCBIFileDic;
	CCData *data = dynamic_cast<CCData *>(CCBIFileDic.objectForKey(strCCBFileName.c_str()));
	if(!data)
	{
		unsigned char * pBytes = CCFileUtils::sharedFileUtils()->getFileData(strCCBFileName.c_str(), "rb", &size);
...
std::string CCBReader::readCachedString() {
	...
	if (n >= this->mStringCache.size())
	{
		CCAssert(false,"string size error");
	}
